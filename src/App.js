...
function App() {
  ...
  //Open the dialog box for getting th user name
  useEffect(() => {
    setOpenWelcomeDialogBox(true);
  }, []);

  //Show the loading symbol until all messages are populated from firebase
  useEffect(() => {
    setLoading(true);
  ...
  //As the page reloads move to bottom
  useEffect(() => {
    scrollToBottom();
  }, [messages]);
  ...
  //Function to scroll to the bottom of all messages
  const scrollToBottom = () => {
    messagesEndRef.current?.scrollIntoView({ behavior: "auto" });
  };

  //Funnction to create the new message and store it in forebase
  ...
  //Function to send message if the enter key is pressed
